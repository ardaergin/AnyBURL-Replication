...
class GeneralizedRule:
    ...
    # Storing timestamps from the bottom rule for reference
    head_timestamp: Optional[float] = field(init=False, default=None)
    body_timestamps: List[Optional[float]] = field(init=False, default_factory=list)
    ...
    def __post_init__(self):
        ...
        if self.rule_type == "AC1" and self.bottom_rule.is_cyclical and \
           (self.AC1_rule_variant not in ["Y_as_constant", "X_as_constant"]):
            raise ValueError("AC1 rules for cyclical paths must specify AC1_rule_variant as either 'Y_as_constant' or 'X_as_constant'")
        if self.rule_type != "AC1" and self.AC1_rule_variant is not None:
            raise ValueError("AC1_rule_variant should only be specified for AC1 rules")
        ...
        # Doing the mapping:
        # (!) We can omit the time in the generalized rule
        ...
        # Just storing the original timestamps for reference
        self.head_timestamp = self.bottom_rule.head.timestamp
        self.body_timestamps = [t.timestamp for t in self.bottom_rule.body]
    ...
    def _bind_triple_variables(self, kg: KnowledgeGraph, triple: Triple, grounding: Dict[str, str]) -> bool:
        """
        Bind subject/object of 'triple' to actual entities in 'kg'
        so that triple is satisfied. If triple has a known timestamp in the original
        bottom rule, you can optionally call kg.has_fact_temporal(...) here.
        """
        subj_key = triple.subject
        obj_key = triple.object
        r = triple.relation

        # Case 1: Both subject and object are already bound
        if subj_key in grounding and obj_key in grounding:
            s_bound = grounding[subj_key]
            o_bound = grounding[obj_key]
            # If the original triple from bottom rule had a timestamp, we can attempt a temporal check
            # We first find the matching bottom-rule triple to get its timestamp
            # (In principle, we can store some index, but let's do a simple approach)
            original_t = self._find_original_timestamp(subj_key, r, obj_key)
            if original_t is not None:
                return kg.has_fact_temporal(s_bound, r, o_bound, timestamp=original_t, tolerance=0)
            else:
                return kg.has_fact(s_bound, r, o_bound)
    ...
    def _find_original_timestamp(self, subj_var: str, relation: str, obj_var: str) -> Optional[float]:
        """
        A helper to find the timestamp (if any) of the corresponding triple in the bottom_rule.body
        that was mapped to (subj_var, relation, obj_var).

        In a more elaborate approach, you'd store direct "old triple => new triple" mappings.
        Here we do a naive search for demonstration.
        """
        for i, original_triple in enumerate(self.bottom_rule.body):
            # The 'generalized_body' triple has the same index i
            # If that triple (body[i]) was mapped to (subj_var, relation, obj_var),
            # we can return its timestamp.
            # Seeing if the mapped subject/obj match the node_mappings
            gen_subj = self.node_mappings[original_triple.subject]
            gen_obj  = self.node_mappings[original_triple.object]
            if (gen_subj == subj_var) and (gen_obj == obj_var) and (original_triple.relation == relation):
                return original_triple.timestamp

        # Also checking if it matches the head triple
        if (self.node_mappings[self.bottom_rule.head.subject] == subj_var
            and self.bottom_rule.head.relation == relation
            and self.node_mappings[self.bottom_rule.head.object] == obj_var):
            return self.bottom_rule.head.timestamp

        return None

    def _check_head_grounding(self, kg: KnowledgeGraph, grounding: Dict[str, Any]) -> bool:
        """
        Once the body is satisfied by 'grounding',
        check if the head triple is satisfied in the KG.
        If the head triple had a timestamp, do a temporal check.
        """
        # Map variables/values
        head_subj_key = str(self.generalized_head.subject)
        head_obj_key  = str(self.generalized_head.object)
        head_rel      = self.generalized_head.relation

        head_subj = grounding.get(head_subj_key, head_subj_key)
        head_obj  = grounding.get(head_obj_key, head_obj_key)

        # Use the original head's timestamp for temporal check
        original_timestamp = self.bottom_rule.head.timestamp
        if original_timestamp is not None:
            return kg.has_fact_temporal(head_subj, head_rel, head_obj,
                                        timestamp=original_timestamp,
                                        tolerance=0)
        else:
            return kg.has_fact(head_subj, head_rel, head_obj)
